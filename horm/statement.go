...
func (s *Query) Source(q string, args ...interface{}) *Query {
	...
	s.Unit.DataType = map[string]int8{}
...
func (s *Query) setData(typ int, data interface{}) *Query {
	var op string
	...
	case reflect.Struct:
		var encodeType int
		if typ == codec.EncodeTypeInsertData {
			encodeType = codec.EncodeTypeInsertData
		} else {
			encodeType = codec.EncodeTypeReplaceData
		}

		d, _ := s.GetCoder().Encode(encodeType, data)
		if d == nil {
			return s
		}
		s.Unit.Data = d.(map[string]interface{})
	...
	case reflect.Slice, reflect.Array:
		var encodeType int
		if typ == codec.EncodeTypeInsertData {
			encodeType = codec.EncodeTypeInsertDatas
		} else {
			encodeType = codec.EncodeTypeReplaceDatas
		}
		...
		s.Unit.Datas = d.([]map[string]interface{})
