...
* 1. 复杂业务拼写sql/es/redis 语句访问DB，可读性差，可维护性差，开发效率低下，不同的数据库需要拼写语句，在 UDAS，通过便捷的 horm ，可以极大提升开发效率，已经支持 mysql、postgresql、clickhouse、es、redis 等协议。
* 2. 统一接入层协议可以极大提升跨部门、项目之间协作的效率，降低沟通成本。
* 3. 所有业务模块单独支持数据库访问，开发成本高，权限分散，不易管理。在 UDAS ，可以在服务端做数据配置化管理，包括接入方授权、分表、日志级别等配置。
* 4. 当有兄弟部门有数据需求，需要单独为其开发接口，效率低下，在 UDAS，可以直接给兄弟部门授权表级别数据权限，兄弟部门可以通过 horm 接入。也可以避免数据这里存一份、那里存一份，降低存储成本，降本增效。
...
type Student struct {
  ...
  Image     []byte    `orm:"image,blob,omitempty" json:"image"`                   //omitempty 忽略零值          
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("student").Column("sex,age,count(1) as cnt").
...
通过 `Order` 函数指定排序。
```go
horm.NewStatement("student").FindAll().Order("age") //ORDER BY age ASC

horm.NewStatement("student").FindAll().Order("age", true) //ORDER BY age DESC

horm.NewStatement("student").FindAll().Order("age DESC", "score ASC") //ORDER BY age DESC, score ASC
```

elastic 按照相关性评分排序
```go
horm.NewStatement("es_student").FindAll().Order("_score", true) //ORDER BY _score DESC
```

### LIMIT、OFFSET
通过 `Limit` 函数去指定 limit 、offset 参数。
```go
horm.NewStatement("student").FindAll().Limit(20) // LIMIT 20

horm.NewStatement("student").FindAll().Limit(20, 50) // LIMIT 20 OFFSET 50
```
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("student").FindAll().Page(2, 5).Exec(ctx, &result)
	...
}
```
...
func Test(ctx context.Context) {
	...
	result := make([]*Student, 0)
	err := horm.NewStatement("es_student").FindAll(where).HighLight(&highLight).Exec(ctx, &result)
	...
}
```
查询结果：
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("es_student").FindAll(where).HighLight(&highLight).Exec(ctx, &highLightResult)
	...
}
```
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("student").InsertMap(data).Exec(ctx, &result)
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("es_student").InsertMap(data).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").InsertStruct(&data).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").InsertStruct(&data).Exec(ctx, &result)
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").InsertStructs(&datas).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := make([]*horm.EsResult, 0)
	err := horm.NewStatement("es_student").InsertStructs(&datas).Eq("_id", ids).Exec(ctx, &result)
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").UpdateMap(data).Eq("userid", 9713).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").UpdateMap(data, where).Exec(ctx, &result)
	...
}
```
...
func Test(ctx context.Context) {
	...
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").UpdateMap(data, where).Exec(ctx, &result)
	...
}
```
生成的请求：
...
func Test(ctx context.Context) {
	...
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").UpdateMap(data).Eq("_id", 2233456).Exec(ctx, &result)
	...
}
```
生成的请求：
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").UpdateStruct(data).Eq("userid", 6348).Exec(ctx, &result)
	...
}
```
...
func Test(ctx context.Context) {
	...
	result := horm.AffectedInfo{}
	err := horm.NewStatement("student").Delete(where).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").Delete(where).Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").Delete().Eq("_id", "w001qIEBL4QnOSO-k-s5").Exec(ctx, &result)
	...
}
```
返回结果：
...
func Test(ctx context.Context) {
	...
	result := horm.EsResult{}
	err := horm.NewStatement("es_student").UpdateMap(data).Eq("_id", 2233456).
		Refresh().        // 更新数据立即刷新
		Exec(ctx, &result)
	...
}
...
func Test(ctx context.Context) {
	result := Student{}
	err := horm.NewStatement("skynet").Prefix("student_").Get("13324").Exec(ctx, &result)
	if horm.IsError(err) {
...
func Test(ctx context.Context) {
	...
	err := horm.NewStatement("skynet").Prefix("student_").Set("13324", data).Exec(ctx)
	if horm.IsError(err) {
		fmt.Println("find student error: %v", err)
		return
	}
...
使用示例：
```go
err := horm.NewStatement("skynet").Expire("student_13324", 3600).Exec(ctx)
```
- `TTL`
  函数用法：
```go
...
err := horm.NewStatement("skynet").TTL("student_13324").Exec(ctx, &ttl)
```
- `EXISTS`
  函数用法：
```go
...
err := horm.NewStatement("skynet").Exists("student_13324").Exec(ctx, &exists)
```
- `DEL`
  函数用法：
```go
...
err := horm.NewStatement("skynet").Del("student_13324").Exec(ctx, &affectNum)
```
## 字符串
- `SET`
  函数用法：
...
err := horm.NewStatement("skynet").Set("test_bool", true).Exec(ctx)
err = horm.NewStatement("skynet").Set("test_int", 78).Exec(ctx)
err = horm.NewStatement("skynet").Set("test_float", 63.2567).Exec(ctx)
err = horm.NewStatement("skynet").Set("test_string", "i am ok").Exec(ctx)
err = horm.NewStatement("skynet").Set("test_struct", data).Exec(ctx)
```
- `SETEX`
  函数用法：
```go
...
err := horm.NewStatement("skynet").SetEX("test_bool", true, 3600).Exec(ctx)
err = horm.NewStatement("skynet").SetEX("test_int", 78, 3600).Exec(ctx)
err = horm.NewStatement("skynet").SetEX("test_float", 63.2567, 3600).Exec(ctx)
err = horm.NewStatement("skynet").SetEX("test_string", "i am ok", 86400).Exec(ctx)
err = horm.NewStatement("skynet").SetEX("test_struct", data, 86400).Exec(ctx)
```
- `SETNX`
  函数用法：
```go
...
err := horm.NewStatement("skynet").SetNX("test_struct", data).Exec(ctx, &success)
```
- `GET`
  函数用法：
```go
...
```go
var bval bool
err := horm.NewStatement("skynet").Get("test_bool").Exec(ctx, &bval)
...
var ival int
err = horm.NewStatement("skynet").Get("test_int").Exec(ctx, &ival)

var fval float64
err = horm.NewStatement("skynet").Get("test_float").Exec(ctx, &fval)

var strval string
err = horm.NewStatement("skynet").Get("test_string").Exec(ctx, &strval)

var structVal Student
err = horm.NewStatement("skynet").Get("test_struct").Exec(ctx, &structVal)
```
- `GETSET`
  函数用法：
```go
...
var lastSet Student
err := horm.NewStatement("skynet").GetSet("test_struct", data).Exec(ctx, &lastSet)
...
```go
var ret int
err := horm.NewStatement("skynet").Incr("test_int").Exec(ctx, &ret)
```
- `DECR`
  函数用法：
```go
...
```go
var ret int
err := horm.NewStatement("skynet").Decr("test_int").Exec(ctx, &ret)
```
- `INCRBY`
  函数用法：
```go
...
```go
var ret int
err := horm.NewStatement("skynet").IncrBy("test_int", 15).Exec(ctx, &ret)
```
- `MSET`
  函数用法：
```go
...
err := horm.NewStatement("skynet").Prefix("user_age_").MSet(mdata).Exec(ctx)
```
结果：
```bash
...
var result []int
keys := []string{"a", "b", "c"}
err := horm.NewStatement("skynet").Prefix("user_age_").MGet(keys).Exec(ctx, &result)
```
...
err := horm.NewStatement("skynet").HSet("student", "13324", data).Exec(ctx)
```
- `HSETNX`
  函数用法：
```go
...
err := horm.NewStatement("skynet").HSetNx("student_age", 13324, 22).Exec(ctx, &success)
```
- `HMSET`
  函数用法：
```go
...
err := horm.NewStatement("skynet").Prefix("student_").HmSet("13324", data).Exec(ctx)
```
结果：
```bash
...
err := horm.NewStatement("skynet").Prefix("student_").HmSetStruct("19827", data).Exec(ctx)
```
结果：
```bash
...
err := horm.NewStatement("skynet").HmGet("student_19827", fields).Exec(ctx, &result)
...
```go
err = horm.NewStatement("skynet").HGet("test_hset", "xxxxxx").Exec(ctx, &bval)
if horm.IsError(err) {
	return err
}
...
err = horm.NewStatement("skynet").HGet("test_hset", "aa").Exec(ctx, &ival)
err = horm.NewStatement("skynet").HGet("test_hset", "bb").Exec(ctx, &fval)
err = horm.NewStatement("skynet").HGet("test_hset", "cc").Exec(ctx, &strval)
err = horm.NewStatement("skynet").HGet("test_hset", "ee").Exec(ctx, &structVal)
```
- `HGETALL`
  函数用法：
```go
...
```go
var result Student //可以通过一个 struct 指针接收结果
err := horm.NewStatement("skynet").HGETALL("student_19827", fields).Exec(ctx, &result)
...
```go
var result []string
err := horm.NewStatement("skynet").Hkeys("student_19827").Exec(ctx, &result)
```
...
```go
var newAge int
err := horm.NewStatement("skynet").HIncrBy("student_19827", "age", 5).Exec(ctx, &newAge)
```
- `HDEL`
  函数用法：
```go
...
```go
var delNum int
err := horm.NewStatement("skynet").HDel("student", 19827, 23312, 98322).Exec(ctx, &delNum)
```
- `HEXISTS`
  函数用法：
```go
...
err := horm.NewStatement("skynet").HExists("student", 19827).Exec(ctx, &exist)
```
- `HLEN`
  函数用法：
```go
...
```go
var num int
err := horm.NewStatement("skynet").HLen("student").Exec(ctx, &num)
```
- `HSTRLEN`
  函数用法：
```go
...
```go
var l int
err := horm.NewStatement("skynet").HStrLen("student_19827", "name").Exec(ctx, &l)
```
- `HINCRBYFLOAT`
  函数用法：
```go
...
```go
var score float64
err := horm.NewStatement("skynet").HIncrByFloat("student_19827", "score", 0.7).Exec(ctx, &score)
```
- `HVALS`
  函数用法：
```go
...
```go
var ages []int
err := horm.NewStatement("skynet").HVals("student_age").Exec(ctx, &ages)
```
## 列表
- `LPUSH`
  函数用法：
...
err := horm.NewStatement("skynet").LPush("student_list", data).Exec(ctx, &n)
```
- `RPUSH`
  函数用法：
```go
...
err := horm.NewStatement("skynet").RPush("student_list", data).Exec(ctx, &n)
```
- `LPOP`
  函数用法：
```go
...
err := horm.NewStatement("skynet").LPop("student_list").Exec(ctx, &result)
```
- `RPOP`
  函数用法：
```go
...
```go
var result Student //用于接收返回的任意类型指针
err := horm.NewStatement("skynet").RPop("student_list").Exec(ctx, &result)
```
...
```go
var n int
err := horm.NewStatement("skynet").LLen("student_list").Exec(ctx, &n)
```
...
err := horm.NewStatement("skynet").SAdd("student_set", data).Exec(ctx, &n)
```
- `SMEMBERS`
  函数用法：
```go
...
```go
var result []*Student
err := horm.NewStatement("skynet").SMembers("student_set").Exec(ctx, &result)
```
- `SREM`
  函数用法：
```go
...
err := horm.NewStatement("skynet").SRem("student_set", data).Exec(ctx, &n)
```
- `SCARD`
  函数用法：
```go
...
```go
var n int // 被成功移除的元素的数量，不包括被忽略的元素。
err := horm.NewStatement("skynet").SCard("student_set").Exec(ctx, &n)
```
- `SISMEMBER`
  函数用法：
```go
...
var isMember bool
err := horm.NewStatement("skynet").SIsMember("student_set", data).Exec(ctx, &isMember)
```
- `SRANDMEMBER`
  函数用法：
```go
...
```go
var result []*Student
err := horm.NewStatement("skynet").SRandMember("student_set", 2).Exec(ctx, &result)
```
- `SPOP`
  函数用法：
```go
...
```go
var result []*Student
err := horm.NewStatement("skynet").SPop("student_set", 2).Exec(ctx, &result)
```
- `SMOVE`
  函数用法：
```go
...
err := horm.NewStatement("skynet").SMOVE("student_set", "student_set2", &data).Exec(ctx)
```
## 有序集
- `ZADD`
  函数用法：
...
err := horm.NewStatement("skynet").ZAdd("student_zset", &data1, 97, &data2, 89).Exec(ctx, &n)
```
- `ZREM`
  函数用法：
```go
...
err := horm.NewStatement("skynet").ZRem("student_zset", &data).Exec(ctx, &n)
```
- `ZREMRANGEBYSCORE`
  函数用法：
```go
...
```go
var n int // 被移除成员的数量。
err := horm.NewStatement("skynet").ZRemRangeByScore("student_zset", 90, 100).Exec(ctx, &n)
```
- `ZREMRANGEBYRANK`
  函数用法：
```go
...
```go
var n int // 被移除成员的数量。
err := horm.NewStatement("skynet").ZRemRangeByRank("student_zset", 0, 2).Exec(ctx, &n)
```
- `ZCARD`
  函数用法：
```go
...
```go
var n int
err := horm.NewStatement("skynet").ZCard("student_zset").Exec(ctx, &n)
```
- `ZSCORE`
  函数用法：
```go
...
var score float64
err := horm.NewStatement("skynet").ZScore("student_zset", &data).Exec(ctx, &score)
```
- `ZRANK`
  函数用法：
```go
...
var rank int
err := horm.NewStatement("skynet").ZRank("student_zset", &data).Exec(ctx, &rank)
```
- `ZREVRANK`
  函数用法：
```go
...
var rank int
err := horm.NewStatement("skynet").ZRevRank("student_zset", &data).Exec(ctx, &rank)
```
- `ZCOUNT`
  函数用法：
```go
...
```go
var n int
err := horm.NewStatement("skynet").ZCount("student_zset", 70, 80).Exec(ctx, &n)
```
- `ZPOPMIN`
  函数用法：
```go
...
err = horm.NewStatement("skynet").ZPopMin("student_zset", 2).Exec(ctx, &result, scores)
```
- `ZPOPMAX`
  函数用法：
```go
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZPopMax("student_zset", 2).Exec(ctx, &result, &scores)
```
- `ZINCRBY`
  函数用法：
```go
...
err = horm.NewStatement("skynet").ZIncrBy("student_zset", &data, 9.5).Exec(ctx, &newScore)
```
- `ZRANGE`
  函数用法：
```go
...
```go
var result []*Student
err = horm.NewStatement("skynet").ZRange("student_zset", 0, 9).Exec(ctx, &result)
```
- `ZRANGE WITHSCORES`
  函数用法：
```go
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRangeWithScore("student_zset", 0, 9).Exec(ctx, &result, &scores)
```
返回结果：
...
err = horm.NewStatement("skynet").ZRangeByScore("student_zset", 80, 90).Exec(ctx, &result)
```
...
err = horm.NewStatement("skynet").ZRangeByScoreWithLimit("student_zset", 80, 90, 0, 10).Exec(ctx, &result)
```
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRangeByScoreWithScoreWithLimit("student_zset", 70, 100, 0, 10).Exec(ctx, &result, &scores)
```
返回结果：
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRangeByScoreWithScore("student_zset", 70, 100).Exec(ctx, &result, &scores)
```
- `ZREVRANGE`
  函数用法：
```go
...
```go
var result []*Student
err = horm.NewStatement("skynet").ZRevRange("student_zset", 0, 9).Exec(ctx, &result)
```
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRevRangeWithScore("student_zset", 0, 9).Exec(ctx, &result, &scores)
```
返回结果：
- `ZREVRANGEBYSCORE`
  函数用法：
...
```go
var result []*Student // 80分~90分之间
err = horm.NewStatement("skynet").ZRevRangeByScore("student_zset", 80, 90).Exec(ctx, &result)
```
...
```go
var result []*Student // 80分~90分之间，OFFSET=0，LIMIT=10
err = horm.NewStatement("skynet").ZRevRangeByScoreWithLimit("student_zset", 80, 90, 0, 10).Exec(ctx, &result)
```
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRevRangeByScoreWithScoreWithLimit("student_zset", 70, 100, 0, 10).Exec(ctx, &result, &scores)
```
...
var result []*Student
var scores []float64 // 返回成员的分数，与 result 数组顺序一一对应。
err = horm.NewStatement("skynet").ZRevRangeByScoreWithScore("student_zset", 70, 100).Exec(ctx, &result, &scores)
```